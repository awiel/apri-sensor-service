
#systemctl stop SCAPE604-apri-sensor-service-pmsa003
#systemctl stop SCAPE604-apri-sensor-service-sps30
#systemctl stop SCAPE604-apri-sensor-service-ds18b20
#systemctl stop SCAPE604-apri-sensor-service-bme680


#systemctl stop SCAPE604-apri-sensor-service-arduinobin.service
systemctl stop SCAPE604-apri-sensor-service-ds18b20.service
systemctl stop SCAPE604-apri-sensor-service-ds18b20-hour.service
systemctl stop SCAPE604-apri-sensor-service-pmsa003.service
systemctl stop SCAPE604-apri-sensor-service-pmsa003-hour.service
systemctl stop SCAPE604-apri-sensor-service-pmsa003-wsaqi.service

systemctl stop SCAPE604-apri-sensor-service-tsi3007-hour.service
systemctl stop SCAPE604-apri-sensor-service-tsi3007.service
systemctl stop SCAPE604-apri-sensor-service-bme280.service
systemctl stop SCAPE604-apri-sensor-service-bme280-hour.service
systemctl stop SCAPE604-apri-sensor-service-dylos.service
systemctl stop SCAPE604-apri-sensor-service-dylos-hour.service

systemctl stop SCAPE604-apri-sensor-service-tgs5042.service
systemctl stop SCAPE604-apri-sensor-service-tgs5042-hour.service

#systemctl stop SCAPE604-apri-sensor-service-sensorcontrol.service
#systemctl stop SCAPE604-apri-server-images.service

systemctl stop SCAPE604-apri-sensor-service-bme680.service
systemctl stop SCAPE604-apri-sensor-service-bme680-hour.service
#systemctl stop SCAPE604-apri-sensor-service-sensorselect.service
#systemctl stop SCAPE604-apri-server-sensorcontrol.service

systemctl stop SCAPE604-apri-sensor-service-ips7100.service
systemctl stop SCAPE604-apri-sensor-service-ips7100-hour.service
systemctl stop SCAPE604-apri-sensor-service-sps30.service
systemctl stop SCAPE604-apri-sensor-service-sps30-hour.service
#systemctl stop SCAPE604-openiod-fiware-connect-josene.service

systemctl stop SCAPE604-apri-sensor-service-caire.service
systemctl stop SCAPE604-apri-sensor-service-caire-hour.service
#systemctl stop SCAPE604-openiod-fiware-connect-server-knmi.service

#systemctl stop SCAPE604-openiod-fiware-connect-ttn-arbaminch.service
