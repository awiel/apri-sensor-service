...
** Module: apri-sensor-service-bme680.js
**   ApriSensorService server
**			inbox service for bme680 sensor data
**
**
*/
...
var service 		= 'apri-sensor-service-bme680';
	log("Path: " + service);
...
app.get('/'+sensorServiceName+'/testservice', function(req, res ) {
	log("ApriSensorService bme680 testservice: " + req.url );
...
app.get('/'+sensorServiceName+'/v1/m', function(req, res) {
	...
	if (req.query.calType) {
		...
		if (req.query.calType=='R') { // R = calibrate on Raw base (particals for bme680)
			calType = 'R';
		}
	}
	var fiwareObject = {};
...
app.get('/*', function(req, res) {
	log("Apri-Sensor-service bme680 request url error: " + req.url );
