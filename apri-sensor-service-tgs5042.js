/*
** Module: apri-sensor-service-tgs5042.js
**   ApriSensorService server
**			inbox service for tgs5042 CO sensor data
**
**
*/
...
var service 		= 'apri-sensor-service-tgs5042';
	log("Path: " + service);
...
app.get('/'+sensorServiceName+'/testservice', function(req, res ) {
	log("ApriSensorService tgs5042 testservice: " + req.url );
...
app.get('/'+sensorServiceName+'/v1/m', function(req, res) {
	...
	if (req.query.calType) {
		...
		if (req.query.calType=='R') { // R = calibrate on Raw base (particals for tgs5042)
			calType = 'R';
		}
	}
	var fiwareObject = {};
...
app.get('/*', function(req, res) {
	log("Apri-Sensor-service tgs5042 request url error: " + req.url );
