/*
** Module: apri-sensor-service-dylos.js
**   ApriSensorService server
**			inbox service for dylos sensor data
**
**
*/
...
var service 		= 'apri-sensor-service-dylos';
	log("Path: " + service);
...
app.get('/'+sensorServiceName+'/testservice', function(req, res ) {
	log("ApriSensorService dylos testservice: " + req.url );
...
app.get('/'+sensorServiceName+'/v1/m', function(req, res) {
	...
	if (req.query.calType) {
		...
		if (req.query.calType=='R') { // R = calibrate on Raw base (particals for dylos)
			calType = 'R';
		}
	}
	var fiwareObject = {};
...
app.get('/*', function(req, res) {
	log("Apri-Sensor-service dylos request url error: " + req.url );
