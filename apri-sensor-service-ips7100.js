...
** Module: apri-sensor-service-ips7100.js
**   ApriSensorService server
**			inbox service for ips7100 sensor data
**
**
*/
...
var service 		= 'apri-sensor-service-ips7100';
	log("Path: " + service);
...
app.get('/'+sensorServiceName+'/testservice', function(req, res ) {
	log("ApriSensorService ips7100 testservice: " + req.url );
...
app.get('/'+sensorServiceName+'/v1/m', function(req, res) {
	...
	if (req.query.calType) {
		...
		if (req.query.calType=='R') { // R = calibrate on Raw base (particals for ips7100)
			calType = 'R';
		}
	}
	var fiwareObject = {};
...
app.get('/*', function(req, res) {
	log("Apri-Sensor-service ips7100 request url error: " + req.url );
