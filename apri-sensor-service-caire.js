...
** Module: apri-sensor-service-caire.js
**   ApriSensorService server
**			inbox service for caire sensor data
**
**
*/
...
var service 		= 'apri-sensor-service-caire';
	log("Path: " + service);
...
app.get('/'+sensorServiceName+'/testservice', function(req, res ) {
	log("ApriSensorService caire testservice: " + req.url );
...
app.get('/'+sensorServiceName+'/v1/m', function(req, res) {
	...
	if (req.query.calType) {
		...
		if (req.query.calType=='R') { // R = calibrate on Raw base (particals for caire)
			calType = 'R';
		}
	}
	var fiwareObject = {};
...
app.get('/*', function(req, res) {
	log("Apri-Sensor-service caire request url error: " + req.url );
